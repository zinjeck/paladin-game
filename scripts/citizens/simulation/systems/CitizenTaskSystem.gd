...
static func _task_is_at_normal_food_safe_boundary(
	...
	var task_phase := str(
		current_task.get("phase", CityCitizens.CITY_CITIZEN_TASK_PHASE_NONE)
	)
	...
	match task_kind:
		...
		CityCitizens.CITY_CITIZEN_TASK_KIND_PLAYER_COMMAND:
			return task_phase != CityCitizens.CITY_CITIZEN_TASK_PHASE_PERFORMING
...
static func _make_work_task_advance_context(
	...
	return {
		...
		"workplace_movement_mode": str(
			movement_policy.get(
				"mode",
				CityObjectCatalog.WORKPLACE_MOVEMENT_MODE_NONE
			)
		),
...
static func _advance_performing_work_task(
	...
	if (
		...
		!= CityObjectCatalog.WORKPLACE_MOVEMENT_MODE_MOVE_BETWEEN_WORK_POINTS
	):
		return path_requests_remaining
...
static func _advance_return_home_task(
	...
	if (
		task_phase == CityCitizens.CITY_CITIZEN_TASK_PHASE_TRAVELING
		and movement_state == CityCitizens.CITY_CITIZEN_MOVEMENT_STATE_MOVING
	):
		return path_requests_remaining
...
static func _prepare_return_home_task_phase(
	...
	if task_phase == CityCitizens.CITY_CITIZEN_TASK_PHASE_BLOCKED:
	...
	if (
		task_phase != CityCitizens.CITY_CITIZEN_TASK_PHASE_PENDING
		and task_phase != CityCitizens.CITY_CITIZEN_TASK_PHASE_TRAVELING
...
static func _set_work_task_activity_state(
	...
	var old_target_tile := CityCitizens.INVALID_CITY_TILE_POSITION
...
static func _set_work_task_blocked(
	...
	var target_tile := CityCitizens.INVALID_CITY_TILE_POSITION
