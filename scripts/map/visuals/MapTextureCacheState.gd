...
static var world_source_ref: WeakRef
static var world_source_instance_id: int = 0
...
static var city_source_ref: WeakRef
static var city_source_instance_id: int = 0
...
static func has_valid_world_cache(source_world) -> bool:
	...
	var cached_world = (
		world_source_ref.get_ref()
		if world_source_ref != null
		else null
	)
	if (
		cached_world == null
		or not is_same(cached_world, source_world)
		or world_source_instance_id != int(source_world.get_instance_id())
	):
		return false
...
static func store_world_cache(
	...
	world_source_ref = weakref(source_world)
	world_source_instance_id = int(source_world.get_instance_id())
...
static func clear_world_cache() -> void:
	...
	world_source_ref = null
	world_source_instance_id = 0
	world_seed = -1
...
static func has_valid_city_cache(
	...
	var cached_city_world = (
		city_source_ref.get_ref()
		if city_source_ref != null
		else null
	)
	if (
		cached_city_world == null
		or not is_same(cached_city_world, source_city_world)
		or city_source_instance_id
		!= int(source_city_world.get_instance_id())
	):
		return false
...
static func store_city_cache(
	...
	city_source_ref = weakref(source_city_world)
	city_source_instance_id = int(source_city_world.get_instance_id())
...
static func clear_city_cache() -> void:
	...
	city_source_ref = null
	city_source_instance_id = 0
	city_seed = -1
