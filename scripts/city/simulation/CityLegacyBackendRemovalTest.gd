...
func _test_single_city_backend_contract() -> void:
	...
	_expect(WorldPoliticalState.get_current_city_runtime_data().get("marker") == "A", "City A must resolve through the sole city backend.")
	_expect(WorldPoliticalState.set_active_settlement(int(b["id"])), "City B must activate.")
	_expect(WorldPoliticalState.get_current_city_runtime_data().get("marker") == "B", "City B must resolve independently.")
