...
# File responsibility: Physical city logistics for an explicit CITY settlement.
...
# Construction, citizens, food, and object storage remain owned by their
# respective systems and are consulted through their explicit APIs.
...
static func make_city_ground_tile_haul_endpoint_for_city_state(
	city_state: CitySettlementSimulationState,
...
static func _can_city_ground_pile_exist_at_tile(
	...
	var has_object := CityObjectSystem.has_city_object_at_tile_for_city_state(
		city_state,
		tile_position
	)
...
static func _release_construction_material_reservations(
	...
	var reservation_snapshot := (
		get_city_haul_reservation_snapshot_for_city_state(city_state)
	)
...
static func get_total_city_ground_pile_resource_amount_for_city_state(
	city_state: CitySettlementSimulationState,
	resource: String
) -> int:
	var total_amount := 0
...
static func reset_city_ground_pile_state_for_city_state(
	city_state: CitySettlementSimulationState
) -> void:
	var logistics_state := get_state_for_city_state(city_state)
	...
	logistics_state.ground_piles.clear()
	logistics_state.ground_pile_index_by_id.clear()
	logistics_state.next_ground_pile_id = 1
	logistics_state.ground_pile_version += 1


static func reset_city_haul_reservation_state_for_city_state(
	city_state: CitySettlementSimulationState
) -> void:
	var logistics_state := get_state_for_city_state(city_state)
	...
	logistics_state.haul_reservations.clear()
	logistics_state.haul_reservation_id_by_citizen_id.clear()
	logistics_state.haul_source_reserved_amount_by_key.clear()
	logistics_state.haul_destination_reserved_amount_by_key.clear()
	logistics_state.next_haul_reservation_id = 1
	logistics_state.haul_reservation_version += 1
	city_state.citizen_task_runtime_state.citizen_task_version += 1
