...
# Resource destinations are scored through one immutable policy boundary.
# A future priority UI must introduce settlement-owned policy state rather than
# mutating process-global gameplay configuration.
...
const RESOURCE_DEMAND_CATEGORY_PRIORITIES: Dictionary = {
...
static func get_resource_demand_category_priority(
	...
	return int(
		RESOURCE_DEMAND_CATEGORY_PRIORITIES.get(
			category,
...
static func get_resource_demand_category_priorities() -> Dictionary:
	return RESOURCE_DEMAND_CATEGORY_PRIORITIES.duplicate(true)
...
static func _find_best_cargo_construction_demand(
	...
	if (
		not raw_city_state is CitySettlementSimulationState
	...
	for raw_site in construction_sites:
		...
		for raw_resource in resources.keys():
			...
			var soft_reserved_space := (
				CityLogisticsSystem.get_city_soft_haul_destination_reserved_resource_amount_for_city_state(
					city_state,
					destination,
					resource,
					citizen_id
		...
		var access_tiles := (
			CityConstructionSystem.get_city_construction_site_access_tiles_for_city_state(
				city_state,
				city_world,
				site,
				citizen_id
...
static func _get_construction_order_priority_rank(
	...
	var work_orders := (
		CityWorkSystem.get_city_work_order_snapshot_for_city_state(city_state)
	)
