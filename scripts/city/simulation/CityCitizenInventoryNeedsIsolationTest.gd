...
func _test_equal_version_city_isolation() -> void:
	...
	var city_a := _create_city("Inventory Needs City A", polity_id, Vector2i(1, 1))
	var city_b := _create_city("Inventory Needs City B", polity_id, Vector2i(8, 8))
	...
	_expect(
		...
		and int(state_a.get("version", -1)) == int(state_b.get("version", -2)),
	...
	_expect(
		...
		and not is_same(state_a.get("citizens_array"), state_b.get("citizens_array"))
		and not is_same(state_a.get("citizen_record"), state_b.get("citizen_record"))
		and not is_same(state_a.get("inventory_identity"), state_b.get("inventory_identity"))
		and not is_same(state_a.get("cargo_identity"), state_b.get("cargo_identity")),
