...
func _test_explicit_renderer_binding_and_presentation_only_ready() -> void:
	...
	var city_a := _create_city("Explicit Renderer A", polity_id)
	var city_b := _create_city("Explicit Renderer B", polity_id)
	...
	var object_a := state_a.object_state.objects[0]
	var object_b := state_b.object_state.objects[0]
